...
# 用户界面获取店铺信息
@app.route("/api/user/warehouse", methods=["GET"])
@cross_origin()
def user_get_warehouse():
    data = db.session.execute(text('select * from warehouse')).fetchall()

    Data = []
    for i in range(len(data)):
        dic = dict(warehouse_name=data[i][0], price=data[i][1], sale=data[i][2])
        Data.append(dic)
    print(Data)
...
def user_addorder():
    rq = request.json
    # 获取各个参数
    warehousename = rq.get("warehouse_name")
    ...
    db.session.execute(text( 'insert into oorder( warehouse_name, order_money, order_way, cons_phone, cons_name, cons_addre,create_time) value("%s", %d, "%s", "%s", "%s", "%s","%s")' % (
            warehousename, ordermoney, orderway, consphone, consname, consaddre, create_time)))
    db.session.commit()
